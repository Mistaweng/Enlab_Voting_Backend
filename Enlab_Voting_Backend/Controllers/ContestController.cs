...
namespace Enlab_Voting_Backend.Controllers
{
	...
	{
		private readonly IContestInterfaceRepo _contestRepo;

		public ContestController(IContestInterfaceRepo contestRepo)
		{
			_contestRepo = contestRepo;
		}

		[HttpPost("register")]
		public async Task<IActionResult> RegisterCandidate([FromBody] ContestantRegistration registrationInfo)
		{
			var result = await _contestRepo.RegisterCandidateAsync(registrationInfo);

			return result ? (IActionResult)Ok("Candidate registered successfully") : BadRequest("Candidate registration failed");
		}

		[HttpPost("pay/{contestantId}")]
		public async Task<IActionResult> PayForParticipation(string contestantId, [FromBody] Transaction paymentInfo)
		{
			var result = await _contestRepo.PayForParticipationAsync(contestantId, paymentInfo.PaymentAmount);

			return result ? (IActionResult)Ok("Payment successful") : BadRequest("Payment failed");
		}

		[HttpGet("contestants")]
		public async Task<IActionResult> GetContestants()
		{
			var contestants = await _contestRepo.GetContestantsAsync();

			return Ok(contestants);
		}

		[HttpGet("vote-result/{contestantId}")]
		public async Task<IActionResult> GetVoteResult(string contestantId)
		{
			var voteResult = await _contestRepo.GetVoteResultAsync(contestantId);
	...
	//	private readonly IContestInterfaceRepo _foodRepo;
